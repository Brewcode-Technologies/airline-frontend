...
function DriverLoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (login.fulfilled.match(result)) {
      if (result.payload.user.role !== 'driver') { setRoleError('Access denied. This portal is for drivers only.'); return; }
          ...
          <div className="p-6">
            ...
            {tab === 'email' ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between text-sm text-gray-500">
                  <p>Don&apos;t have an account?{' '}
                    <Link href="/driver-register" className="text-orange-500 hover:underline font-medium">Sign up</Link>
                  </p>
                  <Link href="/driver-change-password" className="text-orange-500 hover:underline font-medium">Change password</Link>
                </div>
              </form>
            ) : (
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-400 text-center">Only driver accounts will be granted access.</p>
              </div>
            )}
          </div>
        </div>
