...
function AirlineLoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (login.fulfilled.match(result)) {
      if (result.payload.user.role !== 'airline') {
        setRoleError('Access denied. This portal is for airline staff only.');
        return;
      }
            ...
            {tab === 'email' ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-400 text-center">Only airline accounts will be granted access.</p>
              </div>
            )}
          </div>
        </div>
